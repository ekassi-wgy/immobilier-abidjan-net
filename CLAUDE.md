...
- **Prochain lot : 1.10 — fiche annonce** (`/annonces/{slug}-ref{id}` : galerie mosaïque + lightbox, critères structurés, mentions juridiques ivoiriennes, carte, bloc agence « vérifiée », formulaire de contact, WhatsApp, appel, biens similaires, partage). Le socle est prêt : `ListingPresenter::url()` produit déjà ces URL depuis l'accueil et la page de résultats (**elles répondent 404 jusqu'à ce lot**), `ListingRepository` porte les règles de périmètre, Leaflet est auto-hébergé côté public (`assets/vendors/leaflet`). Les demandes de contact du formulaire arrivent au lot 1.11 (`LeadRepository` existe déjà).
...
app/
  ...
  Views/front/        ✅ layouts/app (SEO : canonique, Open Graph, JSON-LD, feuilles et scripts par page) · partials/ (header, footer, hero, search, property-card, search-filters, pagination) · pages/ (home, search, favorites, styleguide, errors/error)
...
public/               DOCUMENT ROOT en production
  ...
  assets/js/          ✅ site.js (en-tête, menu mobile, favoris, recherche) · hero.js (diaporama) · search.js (filtres, tri, carte)
