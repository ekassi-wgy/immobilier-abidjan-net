...
- **Terminés** : phase 1 complète (0.1 à 1.13) · **2.1 SEO** (balises par URL, sitemap, robots, redirections) · **2.2 contenu** (pages, actualités, bannières, blog public). Détail et livrables : `docs/PLAN.md`.
- **Prochain lot : 2.3 — audit de sécurité et performance** : relire CSRF / XSS / injections / uploads / permissions sur l'ensemble des écrans, mesurer et optimiser (cache de pages, minification, images, LCP mobile), puis test de charge avec environ 10 000 annonces factices pour vérifier les filtres et les index. Livrable : rapport d'audit et correctifs.
...
- **Propositions faites, à valider par le client (lot 2.2, migration `0005`)** :
  ...
  - **Textes légaux** : mentions légales, CGU, confidentialité et cookies rédigés au nom de **Weblogy** (Cocody Ambassade, 10 Rue Washington Booker, Abidjan · info@weblogy.com · +225 05 64 00 00 80 · weblogy.com) et **publiés**. Relecture juridique recommandée ; quatre mentions obligatoires restent à compléter (forme juridique, capital, RCCM, directeur de publication, hébergeur).
...
app/
  ...
  Views/cmsadmin/     ✅ layouts/ (app, auth) · partials/ (head, navbar, sidebar, footer, scripts, flash, page-header, status-badge, pagination, empty-state, auth-aside, password-field, field, switch, state-badge, row-actions) · pages/ (dashboard/{staff, agency}, properties/{index,show,form,criteria}, auth, account, geo, catalog/{categories,attributes,features}, sites, agencies/{index,form,profile}, leads/{index,show}, users, partner-requests, exports, settings, seo/{index,form,redirects,redirect-form}, content/{pages,posts,banners,page-form,post-form,banner-form}, errors)
  Views/front/        ✅ layouts/app (SEO : canonique, Open Graph, JSON-LD, feuilles et scripts par page) · partials/ (header, footer, hero, search, property-card, search-filters, pagination, property-gallery, property-contact, field, form-shell, cookie-banner) · pages/ (home, search, property, agencies, agency, contact, partner, submit-property, page, posts, post, favorites, styleguide, errors/error)
...
public/               DOCUMENT ROOT en production
  ...
  assets/js/          ✅ site.js (en-tête, menu mobile, favoris, recherche) · hero.js (diaporama) · search.js (filtres, tri, carte) · property.js (visionneuse, carte, partage)
...
- **Annuaire et profil des agences (lot 1.11)** : `/agences` et `/agences/{slug}`, lectures par `ListingRepository` (`agencies()`, `countAgencies()`, `agencyBySlug()`, `agencyZones()`, `byAgency()`, `agencyCities()`). Périmètre public : `status = 'active' AND deleted_at IS NULL`. Le filtre par ville ou commune retient l'implantation **ou** une zone de couverture déclarée (`agency_zones`). Le nombre d'annonces est **recompté** à chaque lecture (jamais `published_properties_count`, qui peut être en retard après une expiration). RCCM, NCC, email de gestion et comptes de l'agence ne sortent jamais du back-office.
- **Pages éditoriales et légales (lot 1.11)** : table `pages`, servies par `PageController` ; `PageRepository::publishedSlugs()` (en cache) déclare **une route par page publiée**, au-dessus du bloc de recherche — un slug de page ne doit donc jamais reprendre un slug de `transaction_types`, et toute écriture dans `pages` (lot 2.2) doit appeler `flush()`. **Une page non publiée répond 404 et son lien disparaît du pied de page** : le site ne sert jamais de page vide ni de lien mort. La migration `0004` publie « À propos » et « Comment ça marche » (textes de proposition) ; **les quatre pages légales restent non publiées en attendant les textes du client**.
- **Bandeau cookies** : informatif, pas de consentement — le site ne dépose que des cookies nécessaires (session sur les pages à formulaire, `ian_fav`, `ian_seen`) et **aucun traceur**. Le jour où une mesure d'audience tierce est ajoutée, il faut le transformer en véritable demande de consentement (et bloquer le traceur tant qu'elle n'est pas donnée).
