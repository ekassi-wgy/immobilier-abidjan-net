...
- **Terminés** : phase 1 complète (0.1 à 1.13 — socle, multisite, authentification, référentiels, agences, annonces, espace agence, tout le site public, back-office, recette par rôle) · **2.1 SEO** (balises par URL, sitemap, robots, redirections). Détail et livrables : `docs/PLAN.md`.
- **Prochain lot : 2.2 — blog, CMS léger et bannières** : CRUD des pages éditoriales (`pages`, déjà servies par `PageController` — penser à `app()->pages()->flush()` après écriture), actualités (`blog_posts`) avec liste et fiche publiques, bannières (`banners`, placement `home_hero` déjà lu par l'accueil). Le menu déclare déjà `/cmsadmin/pages`, `/cmsadmin/actualites` et `/cmsadmin/bannieres` (404 jusqu'à ce lot), et le pied de page affiche automatiquement toute page publiée.
...
app/
  ...
  Views/cmsadmin/     ✅ layouts/ (app, auth) · partials/ (head, navbar, sidebar, footer, scripts, flash, page-header, status-badge, pagination, empty-state, auth-aside, password-field, field, switch, state-badge, row-actions) · pages/ (dashboard/{staff, agency}, properties/{index,show,form,criteria}, auth, account, geo, catalog/{categories,attributes,features}, sites, agencies/{index,form,profile}, leads/{index,show}, users, partner-requests, exports, settings, seo/{index,form,redirects,redirect-form}, errors)
  Views/front/        ✅ layouts/app (SEO : canonique, Open Graph, JSON-LD, feuilles et scripts par page) · partials/ (header, footer, hero, search, property-card, search-filters, pagination, property-gallery, property-contact, field, form-shell, cookie-banner) · pages/ (home, search, property, agencies, agency, contact, partner, submit-property, page, favorites, styleguide, errors/error)
  ...
  Controllers/        ✅ Controller (base) · Front/ (Controller socle public, HomeController 1.8, SearchController 1.9, PropertyController 1.10, AgencyController · ContactController · PageController 1.11, SitemapController 2.1, FavoriteController) · Cmsadmin/ (Controller de base, Dashboard, Auth, Password, Account, Site, Lead, Geo/{City,Commune,District}, Catalog/{Category,Attribute,Feature}, Agencies/{Agency,AgencyProfile,AgencyAccount,StaffUser,PartnerRequest}, Properties/{Property,PropertyAction,PropertyMedia}, Notification) · Preview/ (PROVISOIRE : page 500 seule)
...
database/             ✅ schema.sql (référence v1, 38 tables) · seed.sql (référentiels CI) · migrations/ (0002 révisions, 0003 icônes des familles, 0004 pages éditoriales) — voir docs/database.md
