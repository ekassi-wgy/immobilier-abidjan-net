    ...
    <form class="im-form" id="agency-form" method="post" enctype="multipart/form-data" action="<?= e(cmsadmin_url($isEdit ? 'agences/' . $agency['id'] : 'agences')) ?>" novalidate>
      ...
      <section class="card im-panel im-form-section" id="identite">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6"><?= $field(['name' => 'partner_type', 'type' => 'select', 'label' => __('partners.partner_type'), 'required' => true, 'value' => $value('partner_type', 'agency'), 'options' => array_combine(App\Controllers\Front\ContactController::PARTNER_TYPES, array_map(static fn (string $type): string => __('front.agencies.types.' . $type), App\Controllers\Front\ContactController::PARTNER_TYPES))]) ?></div>
          <div class="col-md-6"><?= $field(['name' => 'legal_form', 'type' => 'select', 'label' => __('front.partner.legal_form'), 'optional' => true, 'placeholder' => '—', 'options' => array_combine(App\Controllers\Front\ContactController::LEGAL_FORMS, array_map(static fn (string $form): string => __('front.partner.legal_forms.' . $form), App\Controllers\Front\ContactController::LEGAL_FORMS))]) ?></div>
