...
app/
  ...
  Views/cmsadmin/     ✅ layouts/ (app, auth) · partials/ (head, navbar, sidebar, footer, scripts, flash, page-header, status-badge, pagination, empty-state) · pages/ (dashboard, properties, auth, account, geo, catalog/{categories,attributes,features}, sites, agencies, users, partner-requests, errors) · partials auth-aside, password-field, field, switch, state-badge, row-actions
  ...
  Views/emails/       ✅ layout (HTML en ligne) · password-reset, invitation (+ .text)
  Controllers/        ✅ Controller (base) · Front/HomeController (503 « en préparation » jusqu’au lot 1.8) · Cmsadmin/ (Controller de base, Auth, Password, Account, Site, Geo/{City,Commune,District}, Catalog/{Category,Attribute,Feature}, Agencies/{Agency,AgencyAccount,StaffUser,PartnerRequest}) · Preview/ (PROVISOIRE, maquettes)
                      ⬜ Front (Search, Property, Agency, Page, Lead…) + Cmsadmin/ (Dashboard, Properties, Leads, Seo, Settings…)
  ...
  Services/           ✅ SiteRepository, Settings, CountryRepository, GeoRepository, CatalogRepository, AgencyRepository, PartnerRequestRepository, ImageUploader · Auth, UserRepository, PasswordHasher, PasswordReset, LoginThrottle, RateLimiter, ActivityLogger, Mailer, IpAddress · ⬜
bin/
...
public/               DOCUMENT ROOT en production
  ...
  uploads/            ✅ fichiers envoyés, git-ignorés sauf .htaccess (aucune exécution de script) · {pays}/agences/{id}/logo-*.webp
...
- Écrans connectés : le proxy du scratchpad peut relayer le cookie de session d’un fichier `curl -c` (en-tête `Cookie`). Les tests par curl écrivent en base locale : ne rien laisser derrière soi (comptes `@test.local` supprimés, ou base réinstallée depuis `schema.sql` + `seed.sql` si seules des données de test existent).
